...
def sample_from_model(coefficients, generator, n_time, x_init, T, opt):
    ...
    with torch.no_grad():
        for i in reversed(range(n_time)):
            ...
            log_plot(opt.epoch_id, 0, x_0[0], t_time[0], 'x_0_predict', 0)
            x_new = sample_posterior(coefficients, x_0, x, t)
            log_plot(opt.epoch_id, 0, x_new[0], t_time[0], 'x_pos_sample', 0)
            x = x_new.detach()
...
def log_plot(epoch, iteration, image,timestep, graph_to_plot, rank_of_train_loop):
    ...
    if iteration % 100 == 0:
        if graph_to_plot == 'real_data':
            ...
            title = f"Gan prediction x_0' from t={timestep+1}"
        ...
        elif graph_to_plot == 'x_t_sample':
            title = f"Sampled noise about to be fed to generator, t={timestep}"
            step=5+4*rank_of_train_loop
        else:
        ...
        os.makedirs(f'./saved_info/dd_gan/{args.dataset}/{args.exp}/sample', exist_ok=True)
        plot.savefig(f'./saved_info/dd_gan/{args.dataset}/{args.exp}/sample/{name}')
    return
...
def sample_and_test(args):
    ...
    if args.compute_fid:
        ...
        x_t_1 = torch.randn(args.batch_size, args.num_channels,args.image_size, args.image_size).to(device)
        log_plot(args.epoch_id, 0, x_t_1[0], 4, 'x_t_sample', 0)
        fake_sample = sample_from_model(pos_coeff, netG, args.num_timesteps, x_t_1,T,  args)
        fake_sample = to_range_0_1(fake_sample)
        torchvision.utils.save_image(fake_sample, './samples_{}_{}.jpg'.format(args.dataset, args.epoch_id))
